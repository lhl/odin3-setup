...
- Source tracing resolves the ambiguous FEXCore/Box64 controls. With ARM64EC Proton, the 64-bit emulator is fixed to FEXCore and only the 32-bit backend is selectable between FEXCore and WoWBox64. With x86-64 Proton, both displayed emulator fields are fixed to Box64; Bionic launches the x86-64 Wine executable through `box64`. The Glibc launcher likewise invokes Box64 directly and relies on its WoW64 path for 32-bit Windows code rather than starting Box86. An ARM64EC session may use FEXCore for x86-64 portions and the selected backend for a separate 32-bit process, but the translators are not chained for the same process.
