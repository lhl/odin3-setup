...
### Which GameNative CPU translator runs?

FEXCore and Box64 are CPU instruction translators, not Proton replacements or graphics drivers. Proton supplies the Windows compatibility layer; FEXCore or Box64 translates its x86 code to the Odin 3's ARM64 CPU; DXVK/VKD3D and Wrapper/Turnip handle separate graphics layers. The selected Proton/Wine architecture fixes the 64-bit path, while the executable's bitness determines which path is actually exercised:

| Container and runtime | 64-bit Windows code | 32-bit Windows code | What can be controlled |
| --- | --- | --- | --- |
| **Bionic + Proton ARM64EC** | ARM64EC/AArch64 Proton portions run natively; their x86-64 portions use **FEXCore**. **Emulator 64-bit** is locked to FEXCore. | **Emulator 32-bit** selects **FEXCore** (default) or **Box64**; the latter is specifically GameNative's WoWBox64 DLL path. | Select the 32-bit backend and each translator's version/preset in **Edit Container → Emulation**. FEXCore remains the fixed 64-bit path even when 32-bit is set to Box64. |
| **Bionic + Proton x86-64** | **Box64** runs the x86-64 Proton/Wine stack and game code. | **Box64** handles 32-bit code through the WoW64 path; GameNative does not start a separate Box86 process. | Both emulator fields are locked to Box64; the Box64 version/preset apply, while FEXCore controls are hidden/inactive. |
| **Glibc + x86-64 Wine** | **Box64** | **Box64** through WoW64 | There is no FEXCore path or editable emulator selector; use the Box64 version/preset. |

An ARM64EC launch can therefore use FEXCore for a 64-bit game or helper and the selected FEXCore/WoWBox64 backend for a 32-bit process in the same session. The translators are not chained on one process. GameNative also installs/extracts both ARM64EC backend DLL packages at launch, so seeing both versions in the UI or files does not prove both are doing work.

Control the dispatch as follows:

- **General → Wine Version** chooses ARM64EC versus x86-64 Proton and therefore fixes the 64-bit translator.
- **Emulation → Emulator 32-bit** is the only direct FEXCore-versus-Box64 switch, and is editable only with ARM64EC Proton. Under ARM64EC, the displayed **Box64 Version** is a **WoWBox64** package version.
- **FEXCore Version/Preset** affects x86 code routed to FEXCore; **Box64 Version/Preset** affects code routed to Box64. A setting for an unused path has no performance benefit.
- The main `.exe` bitness is decisive, but a game can launch mixed-bitness helpers. GameNative does not clearly label the executable architecture in this screen; determine it with a PE inspection tool on a PC if necessary, or treat the backend switch as a controlled diagnostic rather than guessing from the game's age.

For an Odin 3 title with no better evidence, prefer GameNative v1.1.1's **Bionic + Proton ARM64EC + FEXCore** default. Preserve an exact known config over that general preference. For a suspected 32-bit translation problem, first keep ARM64EC and A/B only **Emulator 32-bit: FEXCore ↔ Box64**. Switching to x86-64 Proton is a broader fallback that replaces the whole Proton architecture and 64-bit translator, so it is not a clean FEX-versus-Box microbenchmark.
...
3. **Update the active CPU translator only for a CPU/translation symptom.** Use the dispatch table above to identify the relevant path. For an ARM64EC/FEXCore baseline, try the latest stable FEXCore while preserving the known-good preset and every other setting; this was decisive for Geometry Wars 3 and part of the efficient Hyper Light Drifter result. If a game boots but crashes while loading a later scene, move the active translator's preset one step toward compatibility before replacing Proton. For a suspected 32-bit-specific problem, A/B the ARM64EC **Emulator 32-bit** choice before changing the whole runtime.
...
6. **Choose Proton by title, not globally.** Proton 10 ARM64EC is the current no-report starting point and the XAudio extraction path. Test Proton 11 for a title-specific improvement. For a demonstrated 32-bit issue, first compare ARM64EC's FEXCore and WoWBox64 32-bit backends; use Proton x86-64/Box64 for a broader mouse, engine, or full-runtime compatibility fallback. Keep the known version when a change regresses the game.
...
9. **Record the final exception.** Keep exact GameNative version, Proton architecture/version, executable bitness when known, 32-bit emulator selection, active FEXCore and/or Box64 version/preset, wrapper, Turnip/System driver, display renderer, DX wrapper, resolution, cap, game branch/executable/arguments, and any environment or redistributable change.
